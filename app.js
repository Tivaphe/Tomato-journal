...
function renderEmptyGarden() {
  return `<div class="empty-state"><div><div class="empty-illustration">${icon("tomato")}</div><h3>${selectedRegion === "Toutes les zones" ? "Votre potager est vide" : "Aucune plante dans cette zone"}</h3><p>Ajoutez vos plants de tomates pour commencer à suivre leur croissance, leurs variétés et leurs récoltes.</p><button class="button primary" data-action="add-plant" type="button">${icon("plus")} Ajouter ma première plante</button></div></div>`;
}
